...
?>
	...
	<?php $header_image_display = get_post_meta( $pid, 'wpl_header_image_display', true); ?>
	<?php $header_image = get_post_meta( $pid, 'wpl_header_image', true); ?>
	<div class="intro intro-small <?php if( $header_image_display == 'off' || !$header_image && empty($default_header_image) ) { echo 'no-bg-img'; } ?>">
		...
		<div class="row">
			<div class="intro-caption">
				<h5><?php _e('What\'s new', 'healthmedical-wpl'); ?></h5>
			
				<h2><?php _e('Latest news', 'healthmedical-wpl'); ?></h2>
			</div><!-- /.intro-caption -->
		</div><!-- /.row -->
